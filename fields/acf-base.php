...
class acf_field_svg_icon extends acf_field
{
    ...
    public $cache_key = 'acf_svg_icon_files1';
    ...
    public function render_field($field)
    {
        ?>
        <input
            class="widefat acf-svg-icon-<?php echo esc_attr($field['type']); ?>"
    ...
    public function parse_svg()
    {
        ...
        foreach ($files as $file) {
            ...
            } elseif ('custom' === $file['type']) {
                $id    = sanitize_title($pathinfo['filename']);
                $out[] = [
                    'id'       => $id,
                    'text'     => self::get_nice_display_text($id),
                    'url'      => Roots\asset("images/icons/{$pathinfo['filename']}.svg")->uri(),
                    'disabled' => false,
                ];
            }
        }
    ...
    {
        $include_media_library = apply_filters('acf_svg_icon_include_media_library', true);

        // Bail early if media library icons disabled
